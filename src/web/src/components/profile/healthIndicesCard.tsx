...
function HealthIndicesCard({ indices, activityLevel, goal, consentGranted }: HealthIndicesCardProps) {
  ...
  if (!consentGranted) {
    return (
      <div className='bg-bg-card rounded-lg shadow-md p-6 mb-6'>
        <h2 className='text-xl font-semibold text-text-header mb-4'>Chỉ số sức khỏe</h2>
        <div className='text-center py-8'>
          <div className='text-text-muted mb-2'>
            <FaHeartbeat className='mx-auto text-4xl' />
          </div>
          <p className='text-text-body'>Vui lòng cấp quyền để xem các chỉ số sức khỏe</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className='bg-bg-card rounded-lg shadow-md p-6 mb-6'>
        ...
        <div className='text-center py-8'>
          ...
          <p className='text-text-body'>Hoàn thiện hồ sơ để xem các chỉ số sức khỏe</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className='bg-bg-card rounded-lg shadow-md p-6 mb-6 border-l-4 border-accent'>
      <div className='flex items-center mb-6'>
        ...
        <h2 className='text-xl font-semibold text-text-header'>Chỉ số sức khỏe</h2>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6'>
        ...
        <div className='bg-bg rounded-lg p-4 border border-border-light hover:shadow-lg transition-shadow duration-200'>
          ...
          <h3 className='text-3xl font-bold text-text-header mb-1'>{indices.bmi.value}</h3>
          <p className='text-sm font-medium text-text-body mb-2'>BMI</p>
          <p className='text-xs text-text-muted leading-relaxed'>{getBMICategoryDescription(indices.bmi.category)}</p>
        </div>
        ...
        <div className='bg-bg rounded-lg p-4 border border-border-light hover:shadow-lg transition-shadow duration-200'>
          ...
          <h3 className='text-3xl font-bold text-text-header mb-1'>{indices.bmr.toLocaleString()}</h3>
          <p className='text-sm font-medium text-text-body mb-2'>BMR (kcal/ngày)</p>
          <p className='text-xs text-text-muted leading-relaxed'>Mức chuyển hóa cơ bản</p>
        </div>
        ...
        <div className='bg-bg rounded-lg p-4 border border-border-light hover:shadow-lg transition-shadow duration-200'>
          ...
          <h3 className='text-3xl font-bold text-text-header mb-1'>{indices.tdee.toLocaleString()}</h3>
          <p className='text-sm font-medium text-text-body mb-2'>TDEE (kcal/ngày)</p>
          <p className='text-xs text-text-muted leading-relaxed'>Tổng năng lượng tiêu hao hằng ngày</p>
        </div>
        ...
        <div className='bg-bg rounded-lg p-4 border border-border-light hover:shadow-lg transition-shadow duration-200'>
          ...
          <h3 className='text-3xl font-bold text-text-header mb-1'>{indices.recommendedCalories.toLocaleString()}</h3>
          <p className='text-sm font-medium text-text-body mb-2'>Khuyến nghị (kcal/ngày)</p>
          <p className='text-xs text-text-muted leading-relaxed'>Dựa trên mục tiêu của bạn</p>
        </div>
      </div>
      ...
      <div className='bg-bg-muted rounded-lg p-6 border border-border-light'>
        <div className='flex items-center mb-4'>
          ...
          <h4 className='font-semibold text-text-header'>Tóm tắt sức khỏe</h4>
        </div>
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-text-body'>
          <div className='space-y-2'>
            <p className='flex items-start'>
              ...
              <span>
                {indices.bmi.value} ({indices.bmi.category}) - {getBMICategoryDescription(indices.bmi.category)}
              </span>
            </p>
            <p className='flex items-start'>
              <span className='font-semibold text-text-header mr-2'>BMR:</span>
              <span>{indices.bmr.toLocaleString()} kcal/ngày - Lượng calo cần khi nghỉ ngơi</span>
            </p>
          </div>
          <div className='space-y-2'>
            <p className='flex items-start'>
              ...
              <span>
                {indices.tdee.toLocaleString()} kcal/ngày - Tổng nhu cầu năng lượng hằng ngày (
                {getActivityLevelDescription(activityLevel)})
              </span>
            </p>
            <p className='flex items-start'>
              <span className='font-semibold text-text-header mr-2'>Mục tiêu:</span>
              <span>
                {goal.charAt(0).toUpperCase() + goal.slice(1)} weight → Khuyến nghị{' '}
                {indices.recommendedCalories.toLocaleString()} kcal/ngày
              </span>
            </p>
          </div>
        </div>
...
export default HealthIndicesCard;
